...
func (s *Store) Get(ctx context.Context, id string) (Task, error) {
	var result Task
	err := s.pool.QueryRow(ctx, `SELECT id, workspace_id, dedupe_key, status, correlation_id, created_at, finished_at FROM tsw_tasks WHERE id = $1`, id).Scan(
		&result.ID, &result.WorkspaceID, &result.DedupeKey, &result.Status, &result.CorrelationID,
		&result.CreatedAt, &result.FinishedAt,
	)
	return result, err
...
func (s *Store) SettleExhausted(ctx context.Context) (bool, error) {
	...
	var item Task
	err = tx.QueryRow(ctx, `
		SELECT id,workspace_id,correlation_id,attempt_count
		FROM tsw_tasks
		...
		&item.ID, &item.WorkspaceID, &item.CorrelationID, &item.AttemptNo,
	)
	if errors.Is(err, pgx.ErrNoRows) {
		return false, nil
	}
	...
	_, err = audit.Write(ctx, tx, audit.Event{
		Type: audit.TaskInterrupted, Actor: audit.ActorSystem, RetentionScopeID: item.WorkspaceID,
...
func (s *Store) ClaimRejected(ctx context.Context, worker string, duration time.Duration) (Task, error) {
	...
	err := s.pool.QueryRow(ctx, `
		WITH candidate AS (
			...
			WHERE task_type='workspace_read'
		...
		RETURNING task.id,task.workspace_id,task.dedupe_key,task.status,task.lease_token,task.attempt_count,task.correlation_id`, worker, duration.String()).Scan(&result.ID, &result.WorkspaceID, &result.DedupeKey, &result.Status, &result.LeaseToken, &result.AttemptNo, &result.CorrelationID)
	return result, err
}
...
func (s *Store) EnqueueExpiredCleanups(ctx context.Context, limit int, period string) (int64, error) {
	...
	result, err := s.pool.Exec(ctx, `
		...
		FROM (
			...
			UNION SELECT workspace_id FROM tsw_tasks WHERE status IN ('succeeded','failed','interrupted') AND finished_at<=now()-interval '7 days'
...
func (s *Store) claim(ctx context.Context, worker string, duration time.Duration, taskType string, route AttemptRoute) (Task, error) {
	...
	err = tx.QueryRow(ctx, `
		...
		FROM candidate WHERE task.id = candidate.id
		RETURNING task.id, task.workspace_id, task.dedupe_key, task.status,
			task.lease_token, task.attempt_count, task.correlation_id`, worker, duration.String(), taskType).Scan(
		&result.ID, &result.WorkspaceID, &result.DedupeKey, &result.Status,
	...
	_, err = audit.Write(ctx, tx, audit.Event{
		Type: audit.TaskStarted, Actor: audit.ActorSystem, RetentionScopeID: result.WorkspaceID,
...
func (s *Store) Settle(ctx context.Context, item Task, status, eventType, outcome, resultCode, stage string) error {
	...
	_, err = audit.Write(ctx, tx, audit.Event{
		Type: event, Actor: audit.ActorSystem, RetentionScopeID: item.WorkspaceID,
