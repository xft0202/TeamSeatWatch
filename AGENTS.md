...
- Treat `E:\python_work\kxj-gpt-reg-go-main` as the implementation baseline for capabilities it already contains. Before designing or coding a ticket, locate the matching reference flow and reuse or port its proven platform calls, parsing, state handling, and failure behavior instead of rewriting them. Adapt that code to TeamSeatWatch's accepted architecture, domain names, generated contracts, persistence ownership, and security boundaries. A rewrite requires a concrete incompatibility recorded in the ticket handoff.
...
- Use one Orca-managed worktree and one Orca Pi writer per implementation ticket. The parent owns scope, risk-based verification, one fresh Standards/Spec review pair, merge, push, and cleanup; do not mix writer protocols within a ticket. Treat agent launch, runtime, or output failures as infrastructure failures: report them, preserve the verified worktree, and stop spawning retry loops.
